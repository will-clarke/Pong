...
pub fn intersects(line_1: &LineSegment, line_2: &LineSegment) -> Option<Vector> {
    ...
    let t_scalar = (q - p) * s / (r * s);
    let u_scalar = (q - p) * r / (r * s);
    ...
    (r * s == 0.0 && (q - p) * r != 0.0) {
        ...
        return None;
    } else if r * s != 0.0 && t_scalar >= 0.0 && t_scalar <= 1.0 &&
        u_scalar >= 0.0 && u_scalar <= 1.0 {
            // segment intersects at point
            // p + tr == q + us
            return Some(p + Vector::new(r.x * t_scalar, r.y * t_scalar))
        }
